...
- `Read the module entry files first` -> `src/utils/markdown.ts`, `src/utils/moduleLabel.ts`, `src/utils/docPaths.ts`, `src/utils/consumers.ts` - These are the strongest module starting points. (evidence: `src/utils/markdown.ts`, `src/utils/moduleLabel.ts`, `src/utils/docPaths.ts`, `src/utils/consumers.ts`)
...
- `utils (src/utils) <- docs (src/docs)` (65 imports)
